/*file name: cdrv_register.c
 * Description: wrapper function around cdev API calls -- Char driver registration.
 * Data structures : 
 ...
 * 3. struct class *virt_class -- the class struct to crateing a new class of devices which can be seen on /sys/class/ folder.
 * 4. struct device *virt_device -- the device struct to creating a new device under/sys/class/virt_class/ folder
 *
 * API's Used:
 * 1. register_chrdev(int majorno,DRV_NAME,struct file_operations*)-- wrapper function around the cdev calls, returns major number if successful. if the input param is 0 then do the dynamic allocation
 *
 *
 ...
 *
 *
 * 6. unregister_chrdev(MAJOR(mydev),DRV_NAME); -- free major number minor number used and driver
 ...
 * 3. Code the module_init with the register_chrdev() and  class and device creation apis
...
#define SUCCESS 0
#define FAIL -1
#define DRV_NAME "cdrv_virt_register"
...
void cdrv_exit(void)
{
	unregister_chrdev(MAJOR(mydev),DRV_NAME);
...
int cdrv_init(void)
{
	...
	mydev = MKDEV(ret,minor);
	if(ret < 0)
		printk("error in registering the device driver\n");
	virt_class = class_create(THIS_MODULE,"virtClassReg");
	virt_device = device_create(virt_class,NULL,mydev,NULL,"virtDevReg");
